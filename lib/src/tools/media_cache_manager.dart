...
class MediaLoadingQueue {
  ...
  static final MediaLoadingQueue _instance = MediaLoadingQueue._internal();
  factory MediaLoadingQueue() => _instance;
  MediaLoadingQueue._internal();
  ...
  Future<Uint8List?> loadImage(
    ...
    if (_loadingPaths.contains(path)) {
      ...
      return MediaCacheManager().getCachedImage(path);
    }
  ...
  Future<void> _loadImageInternal(_LoadRequest request) async {
    try {
      ...
      if (data != null) {
        MediaCacheManager().cacheImage(request.path, data);
      }
